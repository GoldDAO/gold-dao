...
use crate::{CanisterId, TimestampNanos};
...
impl Cryptocurrency {
    ...
    pub fn ledger_canister_id(&self) -> Option<CanisterId> {
        match self {
            ...
            Cryptocurrency::SNS1 => {
                Some(Principal::from_text("zfcdd-tqaaa-aaaaq-aaaga-cai").unwrap())
            }
            Cryptocurrency::CKBTC => {
                Some(Principal::from_text("mxzaz-hqaaa-aaaar-qaada-cai").unwrap())
            }
            Cryptocurrency::CHAT => {
                Some(Principal::from_text("2ouva-viaaa-aaaaq-aaamq-cai").unwrap())
            }
            Cryptocurrency::KINIC => {
                Some(Principal::from_text("73mez-iiaaa-aaaaq-aaasq-cai").unwrap())
            }
            Cryptocurrency::Other(_) => None,
        }
    }
...
impl PendingCryptoTransaction {
    ...
    pub fn user_id(&self) -> Option<Principal> {
        match self {
    ...
    pub fn validate_recipient(&self, recipient: Principal) -> bool {
        match self {
            PendingCryptoTransaction::NNS(t) => match t.to {
                UserOrAccount::Account(a) => {
                    a == AccountIdentifier::new(
                        &recipient.into(),
    ...
    pub fn set_recipient(&mut self, owner: Principal, subaccount: Subaccount) {
        match self {
            PendingCryptoTransaction::NNS(t) => {
                t.to = UserOrAccount::Account(AccountIdentifier::new(&owner, &subaccount))
            }
            PendingCryptoTransaction::ICRC1(t) => {
                t.to.owner = owner;
...
impl From<icrc1::PendingCryptoTransaction> for nns::PendingCryptoTransaction {
    fn from(value: icrc1::PendingCryptoTransaction) -> Self {
        nns::PendingCryptoTransaction {
            ...
            memo: value
                .memo
                .map(|m| ic_ledger_types::Memo(u64_from_bytes(m.0.as_slice()))),
