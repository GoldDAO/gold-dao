...
#[test]
fn test_gldt_reward_claim_happy_path() {
    let test_env = default_test_setup();
    let pic = test_env.pic.borrow();

    let gldt_ledger_id = TokenSymbol::GLDT.get_token_info(true).ledger_id;
    let rewards_canister_id = test_env.rewards_canister_id;
    ...
    let user_1_gldt_balance = balance_of(&pic, gldt_ledger_id, user_1_account);
    tick_n_blocks(&pic, 10);
    assert_eq!(
        user_1_gldt_balance,
        Nat::from(100_000_000_00u64) - Nat::from(TokenSymbol::GLDT.get_token_info(true).fee)
    );
    tick_n_blocks(&pic, 20);
}
