...
use usdg_minter_api::updates::{
    add_margin_to_vault, borrow_from_vault, claim_returns, close_vault, deposit_liquidity,
    open_vault, redeem, repay_debt_to_vault, withdraw_liquidity,
};

generate_pocket_update_call!(open_vault);
generate_pocket_update_call!(close_vault);
generate_pocket_update_call!(claim_returns);
generate_pocket_update_call!(redeem);
