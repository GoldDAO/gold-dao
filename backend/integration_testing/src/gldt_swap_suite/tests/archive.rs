...
mod tests {
    ...
    pub fn get_historic_swaps_works_correctly() {
        ...
        for (swap, expected_id) in res.iter().zip((0..100).rev().map(|i| Nat::from(i + 376u64))) {
        ...
        assert_eq!(res.len(), 100);
        for (swap, expected_id) in res.iter().zip((0..100).rev().map(|i| Nat::from(i + 276u64))) {
            assert_eq!(expected_id, swap.0.1);
        }
        ...
        assert_eq!(res.len(), 100);
        for (swap, expected_id) in res.iter().zip((0..100).rev().map(|i| Nat::from(i + 176u64))) {
            assert_eq!(expected_id, swap.0.1);
        }
