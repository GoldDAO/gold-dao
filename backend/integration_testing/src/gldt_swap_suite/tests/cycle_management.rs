...
mod tests {
    ...
    pub fn gldt_swap_manages_archive_canister_cycles_happy_path() {
        ...
        assert_eq!(archive_canisters.len(), 1);
        let ArchiveCanister {
            canister_id: first_archive_principal,
            ..
        } = archive_canisters.first().unwrap();
        let swap_canister_cycle_balance = pic.cycle_balance(gldt_swap);
        let first_archive_cycle_balance_before_swaps =
            pic.cycle_balance(first_archive_principal.clone());
        ...
        let first_archive_cycle_balance_after_cron =
            pic.cycle_balance(first_archive_principal.clone());
        let second_canister_cycle_balance_after_cron =
            pic.cycle_balance(second_archive_principal.clone());
