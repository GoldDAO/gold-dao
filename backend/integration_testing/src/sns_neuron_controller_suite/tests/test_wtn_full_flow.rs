...
fn test_stake_sns_neuron_happy_path() {
    ...
    let wtn_ledger_canister_id = test_env
        .token_ledgers
        .get("wtn_ledger_canister_id")
        .unwrap()
        .clone();
    ...
    for i in 0..20 {
        let proposal_result = test_env.wtn_sns_test_env.submit_proposal(
            test_env.sns_neuron_controller_id,
            &neuron.id.clone().unwrap(),
        );
    ...
    // let vote_result = test_env.wtn_sns_test_env.vote_on_proposal(
    //     test_env.sns_neuron_controller_id,
    //     &neuron_id,
    //     1,
    //     true,
