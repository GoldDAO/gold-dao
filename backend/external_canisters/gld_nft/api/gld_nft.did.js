export const idlFactory = ({ IDL }) => {
  ...
  const ApprovalResult = IDL.Variant({ 'Ok' : IDL.Nat, 'Err' : ApprovalError });
  const CollectionMetadata = IDL.Record({
    ...
    'icrc7_royalty_recipient' : IDL.Opt(Account__2),
  ...
  UpdateShared.fill(
    IDL.Record({ 'mode' : UpdateModeShared, 'name' : IDL.Text })
  );
  ...
  const Nft_Canister = IDL.Service({
    ...
    'icrc7_owner_of' : IDL.Func([IDL.Nat], [Account__2], ['query']),
    'icrc7_royalty' : IDL.Func([], [IDL.Opt(IDL.Nat16)], ['query']),
    'icrc7_royalty_recipient' : IDL.Func([], [IDL.Opt(Account__2)], ['query']),
    ...
    'icrc7_tokens_of' : IDL.Func([Account__2], [IDL.Vec(IDL.Nat)], ['query']),
    ...
    'stage_nft_origyn' : IDL.Func(
        [IDL.Record({ 'metadata' : CandyShared })],
        [OrigynTextResult],
        [],
      ),
