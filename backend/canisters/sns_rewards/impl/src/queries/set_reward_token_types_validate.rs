    ...
    for (token_string, _) in &args.token_list {
        ...
        match parsed_token_result {
            ...
            Err(e) => {
                ...
                return SetRewardTokenTypesValidateResponse::Error(err_message);
            }
        }
    }

    match serde_json::to_string_pretty(&args) {
        Ok(json) => SetRewardTokenTypesValidateResponse::Success(json),
        Err(e) => SetRewardTokenTypesValidateResponse::Error(format!("invalid payload : {e:?}")),
    }
}
