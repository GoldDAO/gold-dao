use crate::stats::custom_types::{BlockHolder, IndexerType, SmallTX, TimeStats};
...
impl Data {
    ...
    pub fn remove_authorised(&mut self, principal_id: String) -> String {
        ...
        if auth_vec.contains(&principal_id) {
            ...
            let rtn: String =
                String::from("Can't remove - Principal isn't in the list of authorised users");
            return rtn;
        }
    ...
    pub fn add_log(&mut self, text: String) {
        ...
        if &self.canister_logs.len() > &MAX_LOGS {
            self.canister_logs.remove(0);
        }
    }
