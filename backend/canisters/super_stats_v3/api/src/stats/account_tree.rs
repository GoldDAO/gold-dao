...
impl AccountTree {
    ...
    fn update_history_balance(
        ...
        if !self.accounts_history.contains_key(&account_history_key) {
            ...
            let new_balance = match tx_type {
                ...
                TransactionType::Out => previous_day_balance
                    .saturating_sub(stx.value)
                    .saturating_sub(fee),
            };
            ...
            let _ = self.history_latest_balance_cache.insert(
                *account_ref,
                (
                    day_of_transaction,
                    HistoryData {
            ...
            // Update the cache with the latest known history
            let _ = self
                .history_latest_balance_cache
                .insert(*account_ref, (day_of_transaction, ach.clone()));
    ...
    pub fn process_transfer_to(
        ...
        if !self.accounts.contains_key(account_ref) {
            ...
            let error = format!("Error - cannot process transfer to, Block: {}", stx);
            return Err(error);
        }
    }
    ...
    pub fn process_approve_from(
        &mut self,
        account_ref: &u64,
        stx: &SmallTX,
    ) -> Result<String, String> {
        let stx_zero_value = SmallTX { value: 0, ..*stx };
...
#[derive(
    CandidType, StableType, Deserialize, Serialize, Clone, Default, AsFixedSizeBytes, Debug,
)]
...
impl Add for Overview {
    ...
    fn add(self, other: Self) -> Self::Output {
        ...
        Overview {
            ...
            received: (
                received_count1 + received_count2,
                received_amount1 + received_amount2,
            ),
            balance: self.balance + other.balance,
            max_balance: self
                .max_balance
                .max(other.max_balance)
                .max(self.balance + other.balance),
        }
    }
}
...
impl Main {
    pub fn get_overview_by_id(&self, id_string: &String) -> Option<Overview> {
        match self.directory_data.get_ref(id_string) {
            Some(ref_value) => match self.account_data.accounts.get(&ref_value) {
                Some(ac_value) => {
                    let ov = Overview {
