...
fn start_processing_timer(secs: u64) -> String {
    ...
    if init_done == false {
        return String::from(
            "Cannot start timer - No target ledger set. Use init method before starting timer",
        );
    }

    // check if running already
    let is_running = RUNTIME_STATE.with(|s| s.borrow().stats.get_timer_state());
    if is_running == true {
        ret = String::from("Processing timer is already running");
    } else {
        start_processing_time_impl(secs);
        RUNTIME_STATE.with(|s| s.borrow_mut().stats.update_timer(true));
