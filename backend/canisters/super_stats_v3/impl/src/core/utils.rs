...
use super_stats_v3_api::{
    core::constants::{OC_DEV_BOT, OC_DEV_PRINCIPAL},
    runtime::RUNTIME_STATE,
};

pub fn log(text: impl AsRef<str>) {
    RUNTIME_STATE.with(|state| state.borrow_mut().data.add_log(text.as_ref().to_string()));
}

pub async fn canister_call<T, U>(
    canister: &str,
    method: &str,
    args: T,
) -> Result<(U,), (RejectionCode, String)>
where
    T: candid::CandidType,
...
pub async fn critical_err(message: String) {
    let canister = RUNTIME_STATE.with(|s| s.borrow().data.get_self_id());
...
pub fn nat_to_u128(nat: Nat) -> Result<u128, String> {
    match nat.0.to_u128() {
        ...
        None => return Err("Not a valid u128".to_string()),
    }
}
...
pub fn nat_to_u64(nat: Nat) -> Result<u64, String> {
    match nat.0.to_u64() {
        ...
        None => return Err("Not a valid u64".to_string()),
    }
}
