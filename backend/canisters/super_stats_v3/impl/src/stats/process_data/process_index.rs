use super_stats_v3_api::{custom_types::SmallTX, stable_memory::STABLE_STATE};
    ...
    for tx in blocks {
        ...
        if let Some(from_ref) = tx.from {
            ...
            match tx.tx_type {
                ...
                0 => {
                    // process overview
                    let ov = STABLE_STATE.with(|s| {
                        s.borrow_mut()
                            .as_mut()
                            .unwrap()
                            .account_data
                            .process_transfer_from(&from_ref, &tx)
                    });
                    match ov {
                        Err(e) => return Err(e),
                        _ => {}
                ...
                3 => {
                    ...
                    let po = STABLE_STATE.with(|s| {
                        s.borrow_mut()
                            ...
                            .unwrap()
                            .account_data
                            .process_approve_from(&from_ref, &tx)
                    });
                    match po {
                        Err(e) => return Err(e),
                        _ => {}
                ...
                _ => {
                    log(format!(
                        "Error - unknown tx type (process_smtx_to_index). Type: {}",
                        tx.tx_type
                    ));
                    let error = format!(
                        "Error 1 - unknown tx type (process_smtx_to_index). Type: {}",
                        tx.tx_type
                    );
                    return Err(error);
                }
        ...
        if let Some(to_ref) = tx.to {
            match tx.tx_type {
                ...
                _ => {
                    ...
                    let error = format!(
                        "Error 2 - unknown tx type (process_smtx_to_index). Type: {}",
                        tx.tx_type
                    );
                    return Err(error);
                }
