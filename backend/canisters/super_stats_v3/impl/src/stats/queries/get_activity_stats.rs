...
pub use super_stats_v3_api::{
    ...
    stats::queries::get_activity_stats::{
        Args as GetActivityStatsArgs, Response as GetActivityStatsResponse,
    },
};
...
pub fn get_activity_stats(days: GetActivityStatsArgs) -> GetActivityStatsResponse {
    ...
    let mut snapshots = STABLE_STATE.with(|s| {
        s.borrow()
            ...
            .unwrap()
            .activity_stats
            .get_daily_snapshots(days as usize)
    });

    let ac_len = STABLE_STATE.with(|s| s.borrow().as_ref().unwrap().account_data.accounts.len());

    let pr_len = STABLE_STATE.with(|s| s.borrow().as_ref().unwrap().principal_data.accounts.len());
