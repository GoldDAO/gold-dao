...
use gldt_swap_common::{
    ...
    swap::{
        BidFailError, BurnFeesError, DepositRecoveryError, MintError, NotificationError,
        SwapDetailForward, SwapErrorForward, SwapId, SwapInfo, SwapStatus, SwapStatusForward,
        TransferFailReason,
    },
};
...
use origyn_nft_reference::origyn_nft_reference_canister::{
    Account as NftSellerAccount, AskFeature, BidFeature, BidRequest, BidResponseTxnType,
    DepositDetail, EndingType, EscrowRecord, EscrowRequest, IcTokenSpec, ManageSaleRequest,
    ManageSaleResponse, ManageSaleResult, PricingConfigShared, SaleStatusSharedSaleType, TokenSpec,
    WithdrawDescription, WithdrawRequest,
};
...
pub fn forward_swap_validate_notification(swap_id: &SwapId, notification: &SubscriberNotification) {
    let (swap, swap_details) = if let Some(swap_info) =
        read_state(|s| s.data.swaps.get_active_swap(swap_id).cloned())
    {
    ...
    if swap_details.nft_canister != notification.collection {
        ...
        set_notification_error(
            &swap,
            NotificationError::CollectionDoesNotMatch(format!(
    ...
    if let Err(e) = is_valid_seller(
        notification.seller.clone(),
        swap_details.gldt_receiver.owner,
    ) {
...
fn is_valid_seller(
    ...
    match account {
        NftSellerAccount::Account { owner, .. } => {
            ...
            return Err(NotificationError::SellerAndReceiverDoesNotMatch(format!(
        ...
        _ => Err(NotificationError::SellerIsNotPrincipalOrAccount(format!(
            "seller in notification is not a Principal type"
        ))),
    }
}
...
fn set_notification_error(swap_info: &SwapInfo, error: NotificationError) {
    ...
    let mut error_message =
        format!("FORWARD SWAP :: notification validation  :: SwapID = {swap_id:?} :: ");
    match &error {
    ...
    swap_info.update_status(SwapStatus::Forward(SwapStatusForward::Failed(
        SwapErrorForward::NotificationFailed(error),
    )));
    debug!("{error_message}");
}
...
fn validate_sale_id_length(sale_id: &String) -> Result<bool, NotificationError> {
    let len = sale_id.len();
    if len > 250 {
        return Err(NotificationError::SaleIDStringTooLong(format!(
            "The length of the sale id may not pass 1000. it's current length is {len}"
        )));
    }
    Ok(true)
}
        ...
        PricingConfigShared::Ask(Some(features)) => {
            for feature in features {
                match feature {
                    ...
                    AskFeature::Token(val) => match val {
                        TokenSpec::Ic(ts) => {
                            if ts != token_spec {
                    ...
                    AskFeature::Ending(EndingType::Timeout(timeout_value)) => {
                        if timeout_value.clone() != Nat::from(180_000_000_000u64) {
                            return Err(NotificationError::TimeoutInvalid(format!(
...
pub async fn forward_swap_perform_bid_on_nft(
    ...
    .await
    {
        Ok(sale_result) => match sale_result {
                ...
                if let ManageSaleResponse::Bid(bid) = *val {
                    match bid.txn_type {
                        ...
                        _ => {
                            ...
                            swap.update_status(SwapStatus::Forward(SwapStatusForward::BidFail(
        ...
        Err(e) => {
            ...
            swap.update_status(SwapStatus::Forward(SwapStatusForward::BidFail(
                BidFailError::CallError(format!("{e:?}")),
            )));
        }
    }
}
...
pub async fn forward_swap_perform_burn_fees(swap_id: &SwapId) {
    ...
    match retry_async(
        || {
            transfer_token(
                ...
                Some(MemoIcrc(ByteBuf::from(
                    swap.get_swap_id().1 .0.to_bytes_le(),
                ))),
            )
        },
        3,
    ...
    .await
    {
        ...
        Err(e) => {
            ...
            swap.update_status(SwapStatus::Forward(SwapStatusForward::BurnFeesFailed(
...
pub async fn forward_swap_perform_deposit_recovery(swap_id: &SwapId) -> Result<(), ()> {
    ...
    let args = ManageSaleRequest::Withdraw(WithdrawRequest::Escrow(WithdrawDescription {
        ...
        Ok(ManageSaleResult::Ok(_)) => {
            match &original_error {
                ...
                SwapStatusForward::BidFail(e) => {
                    // fail it like normal
                    let _ = &swap.update_status(SwapStatus::Forward(SwapStatusForward::Failed(
                        SwapErrorForward::BidFailed(e.clone()),
                    )));
                    return Ok(());
                }
