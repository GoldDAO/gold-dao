...
use gldt_swap_common::{
    ...
    swap::{
        BurnError, EscrowError, FeeTransferError, NftTransferError, RefundError, SwapDetailReverse,
        SwapErrorReverse, SwapId, SwapInfo, SwapStatus, SwapStatusReverse, TransferFailReason,
    },
};
...
use origyn_nft_reference::origyn_nft_reference_canister::{
    Account as OrigynAccount, EscrowReceipt, InstantFeature, MarketTransferRequest,
    MarketTransferRequestReponseTxnType, PricingConfigShared, SalesConfig,
};
...
pub async fn transfer_to_escrow(swap_id: &SwapId) {
    let (swap, swap_details) =
        if let Some(swap_info) = read_state(|s| s.data.swaps.get_active_swap(swap_id).cloned()) {
            ...
            debug!(
                "REVERSE SWAP :: transfer_to_escrow :: {:?} - can't find swap",
                swap_id
            );
            return ();
        };
    ...
    let amount = swap_details.tokens_to_receive.get().clone() + swap_details.swap_fee.clone()
        - GLDT_TX_FEE * 2;
...
pub async fn transfer_nft(swap_id: &SwapId) {
    let (swap, swap_details) =
        if let Some(swap_info) = read_state(|s| s.data.swaps.get_active_swap(swap_id).cloned()) {
            if let SwapInfo::Reverse(details) = swap_info.clone() {
                ...
                debug!(
                    "REVERSE SWAP :: transfer_nft :: {:?} has no forward swap details",
                    swap_id
                );
                return ();
            }
            ...
            debug!(
                "REVERSE SWAP :: transfer_nft :: {:?} - can't find swap",
                swap_id
            );
            return ();
        };
    ...
    match market_transfer_nft_origyn(
        swap_details.nft_canister,
        MarketTransferRequest {
            token_id: swap_details.nft_id_string.clone(),
            sales_config: SalesConfig {
                broker_id: None,
                pricing: PricingConfigShared::Instant(Some(vec![
                    InstantFeature::FeeAccounts(vec![
    ...
    .await
    {
        Ok(res) => match res {
            origyn_nft_reference::origyn_nft_reference_canister::MarketTransferResult::Ok(
                ...
                match res_ok.txn_type {
                    ...
                    other => {
                        ...
                        swap.update_status(SwapStatus::Reverse(
                            SwapStatusReverse::NftTransferFailed(NftTransferError::TransferFailed(
            ...
            origyn_nft_reference::origyn_nft_reference_canister::MarketTransferResult::Err(e) => {
                trace(&format!("{e:?}"));
                error!("{e:?}");
                swap.update_status(SwapStatus::Reverse(SwapStatusReverse::NftTransferFailed(
    ...
    let (swap, swap_details) =
        if let Some(swap_info) = read_state(|s| s.data.swaps.get_active_swap(swap_id).cloned()) {
            if let SwapInfo::Reverse(details) = swap_info.clone() {
                ...
                debug!(
                    "REVERSE SWAP :: burn_gldt :: {:?} has no forward swap details",
                    swap_id
                );
                return ();
            }
            ...
            debug!(
                "REVERSE SWAP :: burn_gldt :: {:?} - can't find swap",
                swap_id
            );
            return ();
        };
...
pub async fn transfer_fees(swap_id: &SwapId) {
    let (swap, swap_details) =
        if let Some(swap_info) = read_state(|s| s.data.swaps.get_active_swap(swap_id).cloned()) {
            if let SwapInfo::Reverse(details) = swap_info.clone() {
                ...
                debug!(
                    "REVERSE SWAP :: transfer_fees :: {:?} has no forward swap details",
                    swap_id
                );
                return ();
            }
            ...
            debug!(
                "REVERSE SWAP :: transfer_fees :: {:?} - can't find swap",
                swap_id
            );
            return ();
        };
    ...
    .await
    {
        ...
        Err(error_message) => {
            swap.update_status(SwapStatus::Reverse(SwapStatusReverse::Failed(
                SwapErrorReverse::FeeTransferFailed(FeeTransferError::CallError(
                    error_message.clone(),
                )),
            )));
            debug!("REVERSE SWAP :: burn :: Swap Id {swap_id:?} :: error =  {error_message:?}");
...
pub async fn refund(swap_id: &SwapId) {
    let (swap, swap_details) =
        if let Some(swap_info) = read_state(|s| s.data.swaps.get_active_swap(swap_id).cloned()) {
            if let SwapInfo::Reverse(details) = swap_info.clone() {
                ...
                debug!(
                    "REVERSE SWAP :: refund :: {:?} has no forward swap details",
                    swap_id
                );
                return ();
            }
    ...
    let swap_status_reverse = match valid_for_refund(&swap_details) {
        ...
        Err(_) => {
            debug!(
                ...
                swap_id, swap_details.status, "SwapStatusReverse::NftTransferFailed(_)"
            );
            return ();
        }
    };
    ...
    match retry_async(
        || {
            transfer_token(
                ...
                gldt_ledger_id,
                swap_details.tokens_to_receive.get().clone() + swap_details.swap_fee.clone()
    ...
    .await
    {
        ...
        Err(msg) => {
            swap.update_status(SwapStatus::Reverse(SwapStatusReverse::RefundFailed(
                RefundError::CallError(msg.clone()),
            )));
