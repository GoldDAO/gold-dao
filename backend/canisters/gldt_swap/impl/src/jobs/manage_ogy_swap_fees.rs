...
pub fn start_job() {
    run_now_then_interval(Duration::from_millis(MANAGE_FEE_ACCOUNTS), spawn_transfer_job);
}
...
async fn manage_fee_accounts() {
    ...
    if nft_canisters.iter().any(|(_, _, fee_account)| fee_account.is_none()) {
    ...
    match
        icrc1_balance_of(ogy_ledger, Account {
    ...
    for (nft_canister_id, _, account) in nft_canisters {
        ...
        match account {
                                ...
                                ).await
                            {
                                Ok(transfer_res) => {
                                    match transfer_res {
                                        Ok(_) => {
                                            info!(
                                                "MANAGE OGY FEE ACCOUNTS :: topped up fee account"
                                            );
                                            verify_current_ogy_balance(&nft_canister_id).await;
                                        }
                                        Err(e) => {
                                            trace(
                                                &format!("MANAGE OGY FEE ACCOUNTS ERROR :: {e:?}")
                                            );
            ...
            None => {
                trace(&format!("MANAGE OGY FEE ACCOUNTS 1 :: no fee account to transfer to"));
...
async fn verify_current_ogy_balance(nft_canister_id: &Principal) {
        ...
        ).await
    {
        Ok(ok_verify_fee) => {
            match ok_verify_fee {
                ManageSaleResult::Ok(ok_fee_res) => {
                    match ok_fee_res.borrow() {
                        ...
                        _ => {
                            debug!(
                                "MANAGE OGY FEE ACCOUNT :: something else happened {ok_fee_res:?}"
                            );
                        }
                    }
                }
