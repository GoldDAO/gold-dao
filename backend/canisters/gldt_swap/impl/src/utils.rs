...
pub async fn is_nft_in_sale_state(nft_id_string: &String, nft_canister_id: &Principal) -> bool {
    match nft_origyn(nft_canister_id.clone(), nft_id_string).await {
        Ok(res) => match res {
            NftInfoResult::Ok(res_ok) => match res_ok.current_sale {
                Some(current_sale) => match current_sale.sale_type {
                    SaleStatusSharedSaleType::Auction(auction) => match auction.status {
...
pub async fn get_all_user_swap_ids(
    ...
    let (futures, archive_pair): (Vec<_>, Vec<_>) = archives
        ...
        .map(|archive| {
            (
                get_swap_indexes_for_user(archive.canister_id, user),
                archive,
            )
        })
        .unzip();
        ...
        match res {
            Ok(swap_ids) => {
                if let Some(ids) = swap_ids {
                    let with_archive_can: Vec<(SwapId, ArchiveCanister)> =
                        ids.into_iter().map(|id| (id, archive.clone())).collect();
