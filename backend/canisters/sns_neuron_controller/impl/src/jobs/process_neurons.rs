...
async fn run_async() {
    let sns_governance_canister_id = read_state(|state| state.data.sns_governance_canister_id);
    ...
    while continue_scanning {
        ...
        // NOTE: the reason why we need a cycle here is that list_neurons can only return 100 neurons
        // at a time (I also have to doublecheck it). In fact, I'm not sure that we would exceed
        // the limit in any case, but it's better to future proof it
    ...
    // TODO: update here the state with all the new fetched neurons
    let sns_ledger_canister_id = read_state(|state| state.data.sns_ledger_canister_id);

    let mut available_rewards_amount: Nat = Nat::from(0u64);
    for neuron in &neurons {
        let neuron_rewrds =
        // TODO: fix clone here
            fetch_neuron_reward_balance(sns_ledger_canister_id, neuron.id.clone().unwrap()).await;
    ...
    if CLAIM_REWARDS_THRESHOLD > available_rewards_amount {
        for neuron in neurons {
            let args = ogy_sns_rewards_api_canister::claim_reward::Args {
                neuron_id: neuron.id.unwrap(),
                token: String::from("OGY"),
            };
            ogy_sns_rewards_c2c_client::claim_reward(sns_governance_canister_id, &args)
                .await
                .unwrap();
        }
