...
use crate::index::__INDEX;
use crate::state::read_state;
pub use gldt_swap_index_api_canister::get_blocks::Args as GetBlocksArgs;
pub use gldt_swap_index_api_canister::get_blocks::Response as GetBlocksResponse;
use gldt_swap_index_api_canister::index::IndexType;
use gldt_swap_index_api_canister::index::SortBy;
use ic_cdk::update;

#[update]
pub async fn get_blocks(args: GetBlocksArgs) -> GetBlocksResponse {
    ...
    GetBlocksResponse { blocks, total }
}
...
async fn get_blocks_with_filters(
    ...
    for filter in filters {
        let block_ids = __INDEX.with(|index| {
            ...
            if let Some(crate::index::IndexValue(block_ids)) = index_ref.get(filter) {
                block_ids.clone()
            } else {
                Vec::new()
            }
