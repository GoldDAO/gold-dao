...
pub fn add_block_to_index(block: &BlockWithId) -> Result<(), String> {
    ...
    let nfts = block_instance.extract_nfts(data).unwrap_or_default();
    let statuses = block_instance
        .extract_swap_statuses(data)
        .unwrap_or_default();
    ...
    __INDEX.with(|index| {
        ...
        for nft in &nfts {
            let nft_key = IndexType::Nft(nft.clone());
            let account_values = index_mut
                .get(&nft_key)
                .map(|v| v.0.clone())
                .unwrap_or_default();
            let mut d: VecDeque<_> = account_values.into();
            d.push_front(block_id);
            let nft_values: Vec<_> = d.into();
            index_mut.insert(nft_key, IndexValue(nft_values.clone()));
        }

        for status in &statuses {
            let status_key = IndexType::Status(status.clone());
            let status_values = index_mut
                .get(&status_key)
                .map(|v| v.0.clone())
                .unwrap_or_default();
            let mut d: VecDeque<_> = status_values.into();
            d.push_front(block_id);
            let nft_values: Vec<_> = d.into();
            index_mut.insert(status_key, IndexValue(nft_values.clone()));
        }
