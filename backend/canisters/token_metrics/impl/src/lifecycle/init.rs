...
use tracing::info;
use utils::consts::{STAGING_GOLD_1G_CANISTER_ID, STAGING_GOLD_10G_CANISTER_ID, GOLD_1G_CANISTER_ID, GOLD_10G_CANISTER_ID, GOLD_100G_CANISTER_ID, GOLD_1000G_CANISTER_ID};
...
fn init(init_args: Option<InitArgs>) {
    ...
    let gold_nft_canister: Vec<(Principal, u64)> = if args.test_mode {
        ...
        vec!((GOLD_1G_CANISTER_ID, 1), (GOLD_10G_CANISTER_ID, 10), (GOLD_100G_CANISTER_ID, 100), (GOLD_1000G_CANISTER_ID, 1000))
    };

    let env = CanisterEnv::new(args.test_mode);
    let mut data = Data::new(gold_nft_canister);
