...
async fn get_super_stats_balance_of(account: String, is_subaccount: bool) -> Nat {
    ...
    fn log_error(acc: String, err: (RejectionCode, String)) {
        ...
        info!(
            ?error_message,
            "There has been an erorr while fetching the super_stats balance of {acc:?}"
        );
    }

    let result = if is_subaccount {
        match super_stats_v3_c2c_client::get_account_overview(super_stats_canister_id, &account)
            .await
        {
        ...
        match super_stats_v3_c2c_client::get_principal_overview(super_stats_canister_id, &account)
            .await
        {
            Ok(response) => response,
            Err(err) => {
