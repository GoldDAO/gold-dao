...
dfx deploy gldt_fee_compensation --network staging --argument '(opt record {
  ...
  gld_nft_canister_conf=vec{
    record { gld_nft_canister_id = principal "'"$(dfx canister id --network staging gldnft_backend_1g)"'";  weight=1;  last_query_index=0};
    record { gld_nft_canister_id = principal "'"$(dfx canister id --network staging gldnft_backend_10g)"'";  weight=10;  last_query_index=0};
    };
  enabled= true;
