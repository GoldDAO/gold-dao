...
const Form = () => {
  ...
  const handleOnSubmit = (data: FieldValues) => {
    // Open Select dialog to allow user to confirm/deselect NFTs before final confirmation
    dispatchTransferNFTState({
      type: "OPEN_SEND_DIALOG_SELECT",
      value: data.recipient_address as string,
    });
  };

  const handleOpenSelect = () => {
    // Get address from form if available, otherwise use saved address
    const formAddress =
      getValues("recipient_address") || transferNFTState.send_receive_address;
    dispatchTransferNFTState({
      type: "OPEN_SEND_DIALOG_SELECT",
      value: formAddress,
    });
