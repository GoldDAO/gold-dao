...
const RewardsContent = () => {
  ...
  return (
    <div className="px-2 sm:px-3 mb-12 sm:mb-48">
      ...
      {isConnecting && (<section className="flex flex-col items-center justify-center gap-8 sm:min-h-[600px] min-h-[400px]">
          <h3 className="font-bold sm:text-2xl text-lg text-center">Connecting...</h3>
        </section>)}

      { (!isConnecting && isConnected) && (<div className="flex justify-start flex-col gap-3 ">
          <RewardsBalance
            setOgy={setOgy}
            setIcp={setIcp}
            setGold={setGold}
      ...
      { (!isConnecting && !isConnected) && (<section className="flex flex-col items-center justify-center gap-8 sm:min-h-[600px] min-h-[100vh]">
