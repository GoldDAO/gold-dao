...
const NftControls = ({ onSale, token_id, weight }) => {
    ...
    return (
        <Box sx={{ display: 'flex', justifyContent: 'flex-start', alignItems: 'center' }}>
            ...
            {isOnSale &&
                <CancelsaleButton setFeedback={setFeedback} token_id={token_id} weight={weight} setIsOnSale={setIsOnSale} />
            }
            <>
                <SnackBarFeedback handleClose={handleCloseFeedback} open={feedback} text={'Sale Canceled'} />
            </>
        </Box>
    );
};
